...
public class OrderServiceImpl implements OrderService {
    ...
    public void add(Order order) {
        ...
        List<OrderItem> orderItemList = (List<OrderItem>) cartMap.get("orderItemList"); //3）填充订单数据并保存
        ...
        int count = orderMapper.insertSelective(order); //添加订单明细
        ...
        for (OrderItem orderItem : orderItemList) {
            ...
            desc.put(orderItem.getSkuId().toString(), orderItem.getNum().toString());
        }
        //清除Redis缓存购物车数据
        // redisTemplate.delete("Cart_"+order.getUsername());
        skuFeign.deCrCount(desc);
    }
