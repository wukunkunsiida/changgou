...
public class UserController {
    ...
    public Result login(String username, String password, HttpServletResponse response) {
        ...
        if (BCrypt.checkpw(password, user.getPassword())) {
            ...
            token.put("role", "USER");
            token.put("success", "SUCCESS");
            token.put("username", username);
            String jwt = JwtUtil.createJWT(UUID.randomUUID().toString(), JSON.toJSONString(token), null);

            Cookie cookie = new Cookie("Authorization", jwt);
            cookie.setDomain("localHost");
            cookie.setPath("/");
            response.addCookie(cookie);
            return new Result(true, StatusCode.OK, "查询成功", jwt);
        }

        return new Result(true, StatusCode.LOGINERROR, "查询失败");
    }
    ...
    public Result addpoints(Integer points) {
        ...
        return new Result(true, StatusCode.OK, "查询积分成功");
    }
}
